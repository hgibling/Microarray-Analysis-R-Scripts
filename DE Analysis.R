...
write.table(reference.list, paste(subdir.all.preproc.filt, paste(TISSUE, "Reference Gene List.txt", sep=" "), sep=""), quote=F, row.names=F, col.names=F)
...
boxplot(no.duplicates, range=1.5, col=plot.colors, xlab="Array", ylab="Log Probe Intensity", main=paste(TISSUE, "Preprocessed Log Probe Intensity with Annotated Genes", sep=" "))
...
plot(pca.annotated.genes$x, col=pca.colors, pch=20, main=paste(TISSUE, "Preprocessed PCA Plot with Annotated Genes", sep=" "))
...
conditions <- gl(4, 5, 20, label=c("LC", "ROS", "RSV", "ZC"))
...
average.condition.names <- c(rep("LC", 5), rep("ROS", 5), rep("RSV", 5), rep("ZC", 5))
